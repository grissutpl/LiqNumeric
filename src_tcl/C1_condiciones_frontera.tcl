...
# definir las restricciones para los nodos de presión de poro en la base de la columna del suelo
fix 1  0 1 0; # se restrige la translación en y en el nodo 1
fix 3  0 1 0; # se restrige la translación en y en el nodo 3
puts "Finished creating all -ndf 3 boundary conditions..."

# Definir grados de libertad iguales para los nodos de presión de poro.
for {set i 7} {$i <= [expr 3*$nNodeY-2]} {incr i 6} {
    for {set j 1} {$j < $count} {incr j 1} {
		if  {$dryNode($j) == $i } {
			fix $dryNode($j)  0 0 1 ; #definir las restricciones para los nodos de presión de poro sobre el nivel freático
		} elseif {$dryNode($j) == [expr 2+$i] } {
			fix $dryNode($j)  0 0 1 ; #definir las restricciones para los nodos de presión de poro sobre el nivel freático
		} else {
			fix $i 0 0 0 
			fix [expr 2+$i] 0 0 0
...
#restringir ultimo nodo interior
fix [expr 3*$nNodeY-1]  0 0
